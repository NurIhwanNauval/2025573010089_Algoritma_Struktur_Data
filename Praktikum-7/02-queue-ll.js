...
class Queue {
    ...
    enqueue(data) {     //tambah ke BELAKANG - 0(1)
    ...
    dequeue() {     //hapus dari DEPAN - 0(1)
        if (this.isEmpty()) return null;
        const val = this.head.data;
        this.head = this.head.next;
        if (!this.head) this.tail = null; //jika antrian jadi kosong
        this.size--;
        return val;
    }
...
// BFS (Breadth-First Search): jelajah greph/grid level per level
